...
`practice9_stage4b_evalhard174`从`model_3599.pt`继续600次迭代，生成`model_4000.pt`与
`model_4198.pt`。固定174动作、每条3回合的对照结果分别为：

| checkpoint | 成功回合 | 成功率 | 3/3动作 | 0/3动作 |
|---|---:|---:|---:|---:|
| `model_3599.pt` | 479/522 | 91.76% | 153 | 11 |
| `model_4000.pt` | 484/522 | 92.72% | 157 | 10 |
| `model_4198.pt` | 480/522 | 91.95% | 154 | 10 |

独立seed 43、每条10回合复核`model_4000.pt`得到1602/1740，成功率92.07%，其中139条10/10、
10条0/10。该结果确认`model_4000.pt`是当前主检查点；继续训练到`model_4198.pt`已出现轻微遗忘，
后续不得默认使用最终迭代替代中期最佳模型。

## 失败轨迹诊断

逐动作评测器新增两个默认关闭的参数：

- `--failure_trace_frames N`：为失败回合保留终止前最多N个状态帧；0表示关闭。
- `--failure_trace_max_episodes N`：限制写入的失败回合数，避免诊断文件无限增长。

启用后额外原子发布`failure_traces.json`。每条记录绑定动作、回合、终止原因和精确终止帧，包含
30关节参考/实际位置速度、14个跟踪身体参考/实际位置、逐身体Z误差、软限位余量、策略动作、
全部接触传感器力以及终止前窗口。训练器不安装诊断记录器，因此默认训练和普通评测路径不变。

对10条0/10动作进行3回合诊断，30个回合全部由`ee_body_pos`终止，其中8回合同时触发anchor Z。
结果分为两类：

- `005890`、`009223`、`011735`、`012634`在第6至8帧失败。四肢Z误差同步快速增长、接触力为0，
  右腕首先超过0.12m；策略动作绝对值峰值约6.3至7.5。这4条NPZ的第0帧双脚高度约0.261至
  0.263m，基座高度约0.512至0.514m，首次任一足端低于0.05m要到第451至535帧（9.02至
  10.70s）。这不是普通的站立初始姿态，而是长时间腾空/无支撑开头引发的初始化动力学问题。
- `000930`、`001882`、`002585`、`006564`、`012605`、`013604`在第437至810帧失败，三次重复
  的失败帧高度一致，均由左脚Z误差刚越过0.12m触发。这是特定相位的左脚跟踪漂移。

正式诊断产物：

`/root/gpufree-data/datasets/practice9/evaluations/model4000_failure_trace10_eval3_seed48/failure_traces.json`
